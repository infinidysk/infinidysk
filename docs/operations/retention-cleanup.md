...
**Remove Orphaned Files** (Maintenance) deletes WebDAV files that are not linked from the library directory and are no longer tied to a SAB history row. Generated STRM/symlink sidecars owned by an orphaned item are deleted with it. Supports dry run. Schedule optional daily cleanup — set container `TZ`. Direct WebDAV or rclone playback is not a library link, and neither are sidecars under the configured STRM/symlink output directories — those are InfiniDysk's own outputs, not library links, even when the output directory sits inside the Library Directory.
