...
| `rc` | Release candidate for the upcoming version, promoted from `dev` builds after basic regression testing. Preferred for all but the most daring testers. Published to `ghcr.io/infinidysk/infinidysk` only, not Docker Hub or the legacy path. |
