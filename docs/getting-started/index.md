...
| `rc` | Release candidate for the upcoming version, promoted from `dev` builds after basic regression testing. Preferred for all but the most daring testers. Published to `ghcr.io/infinidysk/infinidysk` and Docker Hub, but not the legacy path. |
| `dev` | Work-in-progress snapshots deployed to the maintainer’s non-production instance to validate basic functionality. Expect a noisy tag: frequent, sometimes non-functional builds, with no per-build version numbers. Testing only. Published to `ghcr.io/infinidysk/infinidysk` and Docker Hub, but not the legacy path. |
