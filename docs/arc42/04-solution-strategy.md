...
|---|---|
| Low seek/start latency (QS-1) | The *core* model is on-demand: a layered NNTP client stack (`Clients/Usenet/`) and composable `Stream` classes (`Streams/`) resolve arbitrary byte ranges to specific Usenet articles on demand, decoding yEnc (and optionally AES) inline, rather than a linear download-then-serve model. **Correction from an earlier draft**: this is no longer strictly true system-wide — the fork's own predictive-prefetch feature (`PrefetchCacheService`, triggered by a Jellyfin "playback started" webhook) deliberately *does* bulk-download a full predicted-next episode ahead of time specifically to beat this on-demand path's latency for the common "next episode" case. The two models coexist by design: on-demand for the general/random-seek case, opportunistic full-download for the predictable case, with the prefetched file served as a plain local `FileStream` when present (§6.2). |
...
**These are the current implementation's choices, not this document's recommendation for the
future** — §13 treats each one as genuinely open and proposes concrete alternatives (Rust and
Java/GraalVM backends; an htmx + Web Components frontend replacing React entirely), per explicit
user direction that no technology here is load-bearing on this analysis.
