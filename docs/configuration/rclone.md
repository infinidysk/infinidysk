...
| Folder inside WebDAV | Which part of the tree to expose. `/` is all of it. Each enabled mount needs a different one: rclone cannot refresh the folder listings of two mounts that share it |
...
rclone enforces the ceiling per mount, so with several enabled mounts each one
gets an equal share of it — automatic or set in Settings — and together they
stay within the figure shown. Adding or disabling a mount remounts the others
with their new share.
