...
class BackendClient {
  ...
  public async getQueue(limit: number, start: number = 0): Promise<QueueResponse> {
    const data = await call<{ queue: QueueResponse }>(
      `/api?mode=queue&start=${start}&limit=${limit}`,
      "Failed to get queue",
    );
    return data.queue;
  }
  ...
  public async getHistory(limit: number, start: number = 0): Promise<HistoryResponse> {
    const data = await call<{ history: HistoryResponse }>(
      `/api?mode=history&start=${start}&pageSize=${limit}`,
  ...
  public async getHealthCheckHistory(pageSize?: number): Promise<HealthCheckHistoryResponse> {
    ...
    return await call<HealthCheckHistoryResponse>(
      `/api/get-health-check-history${query}`,
