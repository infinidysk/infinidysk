...
describe("IndexersSettings Prowlarr pull sync", () => {
    ...
    it("reports a failed connection test", async () => {
        mockRoutes({ testConnection: { status: true, connected: false, error: "Authentication failed" } });
        const user = userEvent.setup();
        render(createElement(Harness));
    ...
    it("syncs on demand and applies the returned indexer and profile config", async () => {
        const indexerJson = "{\"Indexers\":[{\"Name\":\"Managed\",\"Url\":\"http://prowlarr:9696/7/api\",\"ApiKey\":\"k\",\"Enabled\":true,\"ProwlarrIndexerId\":7}]}";
    ...
    it("flags an invalid Prowlarr URL and blocks the connection test", async () => {
        ...
        expect(within(prowlarrCard()).getByRole("button", { name: "Test Connection" }))
