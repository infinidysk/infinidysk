...
function parseConfig(raw: string | undefined): IndexerConfig {
  try {
    ...
    return {
      ...
      SearchResultLimit:
        typeof parsed.SearchResultLimit === "number" ? parsed.SearchResultLimit : undefined,
...
export function IndexersSettings({ config, setNewConfig, savedConfig }: IndexersSettingsProps) {
  ...
  const handleSearchLimitChange = useCallback(
    (value: string) => {
      ...
      const next: IndexerConfig = {
        ...indexerConfig,
        SearchResultLimit: n && n > 0 ? n : undefined,
      };
      setNewConfig({ ...config, "indexers.instances": serializeConfig(next) });
    },
    [config, indexerConfig, setNewConfig],
        ...
        <ManagedSetting configKey="api.user-agent" className="sm:col-span-2">
          <div className="flex flex-col gap-1.5">
            ...
            <HelpText>
              ...
              an indexer. Per-indexer overrides take precedence. Leave blank to use the application
              default.
            </HelpText>
          </div>
        </ManagedSetting>
...
function IndexerModal({ show, indexer, onClose, onSave }: IndexerModalProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    onSave({
      ...
      SkipTlsVerification:
        url.trim().toLowerCase().startsWith("https://") && skipTlsVerification ? true : undefined,
