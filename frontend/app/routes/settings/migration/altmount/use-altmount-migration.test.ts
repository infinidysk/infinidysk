...
describe("requestAltmountPathDetection", () => {
  it("asks the backend to inspect the trimmed container path", async () => {
    ...
    try {
      await expect(requestAltmountPathDetection("  /altmount-data/config  ")).resolves.toEqual(
        response,
      );
...
describe("standard Altmount path helpers", () => {
  ...
  it("restores status paths without resetting queue tuning", () => {
    ...
    expect(
      connectFormWithStatusPaths(form, {
...
describe("isMigrationWorkActive", () => {
  it.each<SessionStatus>([
    ...
    "removing_orphans",
    "restoring",
  ])("blocks destructive wizard actions while status is %s", (status) =>
    expect(isMigrationWorkActive(status)).toBe(true),
  );
  ...
  it.each<SessionStatus>([
    ...
    "cancelled",
    "linked",
  ])("allows destructive wizard actions after status is %s", (status) =>
    expect(isMigrationWorkActive(status)).toBe(false),
  );
...
describe("canResetMigration", () => {
  it.each<SessionStatus>([
    ...
    "removing_orphans",
    "restoring",
  ])("blocks Reset Wizard while status is %s", (status) =>
    expect(canResetMigration(status, null)).toBe(false),
  );
...
describe("review mutation state guards", () => {
  it.each<SessionStatus>([
    ...
    "cancelled",
    "linked",
  ])("allows Connect from resting state %s", (status) =>
    expect(canConnectMigration(status)).toBe(true),
  );
  ...
  it.each<SessionStatus>([
    ...
    "removing_orphans",
    "restoring",
  ])("blocks Connect during active state %s", (status) =>
    expect(canConnectMigration(status)).toBe(false),
  );
  ...
  it.each<SessionStatus>([
    ...
    "removing_orphans",
    "restoring",
  ])("blocks Scan from illegal state %s", (status) =>
    expect(canStartScanMigration(status)).toBe(false),
  );
  ...
  it.each<SessionStatus>([
    ...
    "removing_orphans",
    "restoring",
  ])("locks category mapping edits while status is %s", (status) =>
    expect(canEditCategoryMappings(status)).toBe(false),
  );
...
describe("runUiMutation", () => {
  ...
  it("records a rejected mutation and returns false", async () => {
    ...
    await expect(
      runUiMutation(() => Promise.reject(new Error("API rejected the mutation")), recordError),
    ).resolves.toBe(false);
...
describe("loadTableRetainingLastGood", () => {
  it("commits a successful response", async () => {
    ...
    await expect(
      loadTableRetainingLastGood(() => Promise.resolve(["fresh"]), commit, recordError),
    ).resolves.toBe(true);
    expect(commit).toHaveBeenCalledWith(["fresh"]);
    expect(recordError).not.toHaveBeenCalled();
  });
