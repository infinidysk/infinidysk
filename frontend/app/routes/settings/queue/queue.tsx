...
export function QueueSettings({ config, setNewConfig }: QueueSettingsProps) {
    ...
    return (
        <SettingsPage>
            <SettingsIntro>
                Control how many NZBs can wait and process at once, and how much of your provider
                connection capacity queue imports may use.
            </SettingsIntro>
                ...
                >
                    <ManagedSetting configKey="queue.worker-count">
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-base-content" htmlFor="queue-worker-count-select">
                            ...
                            <p className="text-[11px] leading-relaxed text-base-content/45" id="queue-worker-count-help">
                                ...
                                preferred access; additional items use spare capacity. Raising this does not
                                increase Queue Download Connections.
                            </p>
                        </div>
                    </ManagedSetting>
                    ...
                    <ManagedSetting configKey="usenet.max-queue-connections">
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-base-content" htmlFor="max-queue-connections-input">
                            ...
                            <p className="text-[11px] leading-relaxed text-base-content/45" id="max-queue-connections-help">
                                Connections shared by queue workers and background health checks. Leave blank
                                to use all Pool provider connections. Streaming has its own connection allocation
                                and controls how capacity is shared when playback and imports overlap.
                            </p>
                        </div>
                    </ManagedSetting>
                </SettingsCard>
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <p className="text-[11px] leading-relaxed text-base-content/45" id="queue-max-items-help">
                                    Reject new SAB submissions when this many jobs are queued. Radarr and Sonarr
                                    keep rejected grabs pending and retry later. Use <code>0</code> for no limit.
                                </p>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-[11px] leading-relaxed text-base-content/45" id="queue-resume-threshold-help">
                                    After the limit is reached, accept submissions again at or below this queue
                                    depth. Use <code>0</code> to resume immediately below the maximum.
...
export function isQueueSettingsUpdated(
    ...
    return config["queue.worker-count"] !== newConfig["queue.worker-count"]
...
export function isQueueSettingsValid(config: Record<string, string>): boolean {
    return isValidQueueWorkerCount(config["queue.worker-count"])
