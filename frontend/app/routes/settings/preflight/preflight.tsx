...
export function PreflightSettings({ config, setNewConfig }: PreflightSettingsProps) {
  ...
  return (
    <SettingsPage>
      <SettingsIntro>
        When a client asks for the list of available articles, InfiniDysk can quietly do upfront
        work on the top-ranked ones so the next request reuses that warm state instead of redoing
        everything from scratch. The harder the mode, the more it does.
      </SettingsIntro>
      ...
      >
        <ManagedSetting configKey="preflight.mode">
          <Field>
            ...
            >
              ...
              <option value="full">
                full — standard + resolve archive layout for previously completed items
              </option>
            </Select>
            <p className="m-0 text-[11px] leading-relaxed text-base-content/45">
              ...
              <b> full</b> additionally resolves trailing-archive metadata for any top result that
              maps to a previously completed item — useful when re-opening something.
            </p>
          </Field>
        </ManagedSetting>
        ...
        <ManagedSetting configKey="preflight.max-attempts">
          <Field>
            ...
            />
            <p className="m-0 text-[11px] leading-relaxed text-base-content/45">
              Walks the top-ranked results one at a time and stops on the first one that passes the
              check. So a missing top result automatically falls through to the next one — same idea
              as the watchdog at click time, but in the background. Default 20.
            </p>
          </Field>
        </ManagedSetting>
        ...
        <ManagedSetting configKey="preflight.ttl-seconds">
          <Field>
            ...
            />
            <p className="m-0 text-[11px] leading-relaxed text-base-content/45">
              How long a preflighted result stays warm before it's discarded. Long enough to scroll
              through and pick something, short enough not to hold stale state. Default 120.
            </p>
          </Field>
        </ManagedSetting>
        ...
        <ManagedSetting configKey="preflight.indexer-max-wait-seconds">
          <Field>
            ...
            />
            <p className="m-0 text-[11px] leading-relaxed text-base-content/45">
              Preflight is best-effort: if an indexer's rate limit would force it to wait longer
              than this before a request can fire, preflight on that result is skipped. Keeps real
              requests from being queued behind speculative work. Default 5.
            </p>
          </Field>
        </ManagedSetting>
      </SettingsCard>
...
export function isPreflightSettingsUpdated(
  config: Record<string, string>,
  newConfig: Record<string, string>,
) {
  return (
    config["preflight.mode"] !== newConfig["preflight.mode"] ||
