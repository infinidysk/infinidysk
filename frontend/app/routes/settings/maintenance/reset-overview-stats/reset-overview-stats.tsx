...
export function ResetOverviewStats() {
  ...
  useEffect(() => {
    ...
    fetch(withUrlBase("/api/get-provider-usage"))
      .then((r) =>
        r.ok
          ? (r.json() as Promise<ProviderUsageResponse>)
          : Promise.reject(new Error("Failed to load provider usage")),
      )
      .then((data) => {
        if (cancelled) return;
        setProviders(
  ...
  return (
    <div className="space-y-4">
      <Alert className="alert-soft items-start py-3 text-sm" variant="warning">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs opacity-80">
            Overview metrics are permanently removed. Queue, history, and provider data-cap gauges
            are preserved. A per-provider reset leaves global throughput charts and session history
            intact.
          </p>
        </div>
      </Alert>
      ...
      <p className="text-sm leading-relaxed text-base-content/70">
        Clear Overview statistics for all providers or a single provider. May take a few moments on
        large databases.
      </p>
      ...
      <div className="space-y-2">
        <label
          ...
          htmlFor="reset-overview-stats-provider"
        >
          Scope
        </label>
        <Select
