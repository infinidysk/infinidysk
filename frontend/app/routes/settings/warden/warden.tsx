...
function kindMeta(kind: Source["kind"]) {
  if (kind === "local")
    return { label: "Local", cls: "text-success border-success/30 bg-success/10" };
...
export function WardenSettings({ config, setNewConfig }: WardenSettingsProps) {
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      setMessage({
        text: `Imported ${(data.added ?? 0).toLocaleString()} fingerprint${data.added === 1 ? "" : "s"}.`,
        variant: "success",
      });
      setShowImport(false);
      setPendingFile(null);
  ...
  const doRestore = async () => {
    ...
    setMessage(null);
    try {
      const data = await post<WardenBackupMutateResponse>(
        "/api/warden-backup-restore",
        new FormData(),
      );
  ...
  return (
    <SettingsPage>
      <SettingsIntro>
        A portable filter list of dead-release fingerprints. Your own list fills in automatically
        and stays independent. You can also add remote lists from a URL; they refresh on their own
        schedule, never touch your own list, and you decide how much each one is trusted.
        Fingerprints are universal: identical on any provider, indexer, or server, and free of
        credentials.
      </SettingsIntro>
      ...
      <ManagedSetting configKey="warden.quorum">
        <Form.Group className={"flex flex-col gap-2"}>
          ...
          <Form.Control
            ...
            value={quorum}
            onChange={(e) =>
              set("warden.quorum", String(Math.max(1, parseInt(e.target.value || "1", 10))))
            }
          />
          <p className={"m-0 text-[11px] leading-relaxed text-base-content/45"}>
            ...
            independent sources agree. Your own list and “full”-trust sources always filter on their
            own.
          </p>
        </Form.Group>
      </ManagedSetting>
      ...
      <div className={`${"flex flex-col gap-2"} ${"w-full"}`}>
        ...
        <p className={"m-0 text-[11px] leading-relaxed text-base-content/45"}>
          Each source has a trust level. <b>full</b> filters on its own; <b>corroborate</b> filters
          only when enough sources agree (the number above); <b>observe</b> keeps the list but never
          filters.
        </p>
        ...
        >
          {sources.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className={`${"rounded-lg border border-base-content/10 bg-base-100 p-3.5 transition-[border-color,opacity] duration-120 hover:border-base-content/20"} ${!s.enabled ? "opacity-55" : ""}`}
              >
                <div className={"flex items-center gap-2.5"}>
                  <div className={"flex min-w-0 flex-1 items-center gap-2.5"}>
                    {isLocal ? (
                      ...
                      <input
                        className={
                          "min-w-0 max-w-[240px] rounded-md border border-transparent bg-transparent px-1.5 py-0.5 text-sm font-semibold text-base-content hover:border-base-content/10 focus:border-base-content/10 focus:bg-base-200 focus:outline-none"
                        }
                        defaultValue={s.name}
                        disabled={rowBusy}
                        key={`nm-${s.id}-${s.name}`}
                        onBlur={(e) => {
                          ...
                          if (v && v !== s.name)
                            void updateSource(s.id, {
                              name: v,
                            }); /* failures surface via setMessage */
                        }}
                        onKeyDown={(e) => {
                    ...
                    <span
                      className={`${"shrink-0 rounded-md border border-base-content/10 bg-base-200 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-base-content/80"} ${km.cls}`}
                    >
                      {km.label}
                    </span>
                ...
                {s.url && (
                  <div className={"mt-1.5 break-all font-mono text-[11px] text-base-content/60"}>
                    {s.url}
                  </div>
                )}
                ...
                <div
                  className={
                    "mt-3 flex flex-wrap items-center gap-4 border-t border-base-content/10 pt-3"
                  }
                >
                  <span className={"text-xs text-base-content/80"}>
                    {s.count.toLocaleString()} fingerprints
                  </span>
                  {isLocal ? (
                    ...
                    <div className={"flex items-center gap-1.5"}>
                      ...
                      <Form.Select
                        className={"w-[123px] shrink-0 text-xs"}
                        value={s.trust}
                        disabled={rowBusy}
                  ...
                  {!isLocal && (
                    <Tooltip content="Include fingerprints from this remote source in filtering.">
                      <Form.Check
                        ...
                        disabled={rowBusy}
                        onChange={(e) =>
                          void updateSource(s.id, { enabled: String(e.target.checked) })
                        }
                      />
                    </Tooltip>
                  )}
                  ...
                  {s.kind === "remote" && (
                    <div className={"flex items-center gap-1.5"}>
                      ...
                      <Form.Control
                        ...
                        key={`rh-${s.id}-${s.refreshHours}`}
                        defaultValue={s.refreshHours}
                        disabled={rowBusy}
                        onBlur={(e) => {
                          const v = parseInt(e.target.value, 10);
                          if (v && v !== s.refreshHours)
                            void updateSource(s.id, {
                              refreshHours: String(v),
                            }); /* failures surface via setMessage */
                        }}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") e.currentTarget.blur();
                  ...
                  {s.kind === "remote" && (
                    <span
                      className={`${"ml-auto text-[11px] text-base-content/60"} ${statusErr ? "text-error" : ""}`}
                    >
                      updated {ago(s.lastUpdated)}
                      {s.status ? ` · ${s.status}` : ""}
                    </span>
                  )}
                </div>
      ...
      <div className={`${"flex flex-col gap-2"} ${"w-full"}`}>
        <div className={"flex items-start justify-between gap-3 mb-1"}>
          ...
          <div className={"flex shrink-0 flex-wrap gap-2"}>
            ...
            <Button
              variant="primary"
              size="xsmall"
              disabled={!backup?.hasToken || busy !== null}
        ...
        <p className={"m-0 text-[11px] leading-relaxed text-base-content/45"}>
          A personal backup of your fingerprint list to a repo you own. Keep the repo <b>private</b>{" "}
          for a pure backup, or public to share. The file holds only fingerprint hashes, no
          credentials. Restore reuses the same repo and token, so private repos work too.
        </p>
      ...
      {message && (
        <Alert
          ...
          className={`${"max-w-[760px] m-0"} flex items-center justify-between gap-3`}
        >
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setShowAddRemote(false)}>
              Cancel
            </Button>
            <Button
      ...
      >
        <Form.Group className={"mb-3.5"}>
          ...
          <Form.Control
            type="url"
            placeholder="https://raw.githubusercontent.com/…/backup.ndjson.gz"
            value={remoteUrl}
        ...
        <div className={"flex gap-3"}>
          <Form.Group style={{ flex: 1 }}>
            ...
            <Form.Select
              value={remoteTrust}
              onChange={(e) => setRemoteTrust(e.target.value as Trust)}
            >
      ...
      >
        <Form.Group className={"mb-3.5"}>
          ...
          />
          <Form.Text muted>
            Or choose a file below. Lines starting with # are ignored. Ones you already have are
            skipped.
          </Form.Text>
        </Form.Group>
        <div className={"flex items-center gap-2.5"}>
      ...
      >
        <Form.Check
          ...
          name="import-target"
          id="import-separate"
          label="Keep as a separate source (recommended)"
          checked={importTarget === "separate"}
          onChange={() => setImportTarget("separate")}
        />
        <div className={"mb-3 ml-[26px] mt-0.5 text-[13px] text-base-content/60"}>
          Stays isolated and reversible: one click to remove later. Best for lists from other
          people.
        </div>
        <Form.Check
          type="radio"
          name="import-target"
          id="import-merge"
          label="Merge into my list"
          checked={importTarget === "merge"}
          onChange={() => setImportTarget("merge")}
        />
        <div className={"mb-3 ml-[26px] mt-0.5 text-[13px] text-base-content/60"}>
        ...
        {importTarget === "separate" && (
          <div className={"flex gap-3"} style={{ marginBottom: 14 }}>
            ...
            <Form.Group style={{ width: 180 }}>
              ...
              <Form.Select
                value={importTrust}
                onChange={(e) => setImportTrust(e.target.value as Trust)}
              >
                <option value="corroborate">corroborate</option>
                <option value="full">full</option>
                <option value="observe">observe</option>
      ...
      >
        <Form.Check
          ...
          id="export-local"
          label="My list only"
          checked={exportScope === "local"}
          onChange={() => setExportScope("local")}
        />
        <div className={"mb-3 ml-[26px] mt-0.5 text-[13px] text-base-content/60"}>
        ...
        <Form.Check
          ...
          name="export-scope"
          id="export-merged"
          label="Merged from selected sources"
          checked={exportScope === "merged"}
          onChange={() => setExportScope("merged")}
        />
        {exportScope === "merged" && (
        ...
        <Tooltip content="Collapse identical fingerprints into a single export line.">
          <Form.Check
            type="switch"
            id="export-dedup"
            label="Deduplicate identical fingerprints"
            className="cursor-pointer gap-2 p-0"
            checked={exportDedup}
            onChange={(e) => setExportDedup(e.target.checked)}
            style={{ marginTop: 8 }}
          />
        </Tooltip>
      </Modal>
      ...
      >
        <Form.Group className={"mb-3.5"}>
          ...
          <Form.Control
            ...
            value={bRepo}
            onChange={(e) => setBRepo(e.target.value)}
          />
        ...
        <Form.Group className={"mb-3.5"}>
          ...
          <Form.Control
            ...
            placeholder={
              backup?.hasToken ? "•••••••• (stored, leave blank to keep)" : "Fine-grained PAT"
            }
            value={bToken}
        ...
        <div className={"flex gap-3"}>
          <Form.Group style={{ flex: 1 }}>
            ...
            <Form.Control
              ...
              placeholder="warden/warden.ndjson.gz"
            />
          </Form.Group>
          <Form.Group style={{ width: 130 }}>
        ...
        <div className={"flex gap-3"}>
          <Form.Group style={{ flex: 1 }}>
            ...
            <Form.Select
              value={bScope}
              onChange={(e) => setBScope(e.target.value as "local" | "merged")}
            >
        ...
        <Tooltip content="Push an automatic backup of your list to GitHub on the schedule below.">
          <Form.Check
            type="switch"
            id="backup-enabled"
            label="Back up automatically on this schedule"
            className="cursor-pointer gap-2 p-0"
            checked={bEnabled}
...
export function isWardenSettingsUpdated(
  config: Record<string, string>,
  newConfig: Record<string, string>,
) {
  return (
    config["warden.hide-dead"] !== newConfig["warden.hide-dead"] ||
