...
export function SabnzbdSettings({ config, setNewConfig }: SabnzbdSettingsProps) {
    ...
    return (
        <SettingsPage>
            <SettingsIntro>
                Configure the SABnzbd-compatible API used by Radarr and Sonarr, then choose how
                submitted NZBs are imported, checked, and backed up.
            </SettingsIntro>
            ...
            >
                ...
                <ManagedSetting configKey="api.categories">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="categories-input">
                            Categories
                        </label>
                        <TagInput
                ...
                <ManagedSetting configKey="api.manual-category">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="manual-category-input">
                ...
                <ManagedSetting configKey="api.addurl-trusted-hosts">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="addurl-trusted-hosts-input">
                            Trusted local hosts
                        </label>
                        <ExpandingTextInput
                            className="w-full"
                        ...
                        <p className="text-[11px] leading-relaxed text-base-content/45" id="addurl-trusted-hosts-help">
                            ...
                            addresses. List trusted hostnames, IP addresses, or CIDR ranges that should be allowed.
                            Use <code>*</code> only when every non-public address is under your control.
                        </p>
                    </div>
                </ManagedSetting>
            </SettingsCard>
            ...
            >
                <ManagedSetting configKey="api.import-strategy">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="import-strategy-input">
                            Import Strategy
                        </label>
                        <Select
                            className="w-full"
                        ...
                        <p className="text-[11px] leading-relaxed text-base-content/45" id="import-strategy-help">
                            Use symlinks for Plex with an rclone WebDAV mount. Use STRM files for Emby or
                            Jellyfin when those servers can stream directly from InfiniDysk.
                        </p>
                    </div>
                </ManagedSetting>
                ...
                {config["api.import-strategy"] === "strm" && (
                    <div className="ml-4 space-y-4 border-l border-base-content/10 pl-4">
                        <ManagedSetting configKey="api.completed-downloads-dir">
                            <div className="space-y-2">
                                <label className="block text-sm font-medium text-base-content" htmlFor="completed-downloads-dir-input">
                        ...
                        <ManagedSetting configKey="general.base-url">
                            <div className="space-y-2">
                                <label className="block text-sm font-medium text-base-content" htmlFor="base-url-input">
                                    Base URL
                                </label>
                                <Input
                                    className="w-full"
                ...
                <ManagedSetting configKey="api.download-file-blocklist">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="ignored-files-input">
                            Ignored Files
                        </label>
                        <TagInput
                            className="w-full"
                ...
                <ManagedSetting configKey="api.duplicate-nzb-behavior">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-base-content" htmlFor="duplicate-nzb-behavior-input">
            ...
            >
                <ManagedSetting configKeys={[
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-base-content" htmlFor="nzb-backup-location-input">
                                Backup directory
                            </label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-base-content" htmlFor="nzb-backup-retention-days-input">
...
function useEnsureArticleExistenceSetting(
    ...
    const categories = useMemo(() => {
        const configured = categoriesValue?.trim()
            ? categoriesValue.split(",").map(category => category.trim()).filter(Boolean)
    ...
    const healthCheckCategories = useMemo(
        () => healthCheckCategoriesValue?.trim()
            ? healthCheckCategoriesValue.split(",").map(category => category.trim()).filter(Boolean)
            : [],
    ...
    const areNoneSelected = healthCheckCategories.length === 0
        || categories.every(category => !healthCheckCategories.includes(category));
...
function isValidCategories(categories: string | undefined): boolean {
