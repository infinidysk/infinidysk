...
export function parseMounts(value: string | undefined): BuiltinMount[] {
  ...
  try {
    ...
    return parsed
      ...
      .map((entry) => ({ ...MOUNT_DEFAULTS, ...withoutUnusableValues(entry) }) as BuiltinMount);
  } catch {
    return [];
  }
}
...
/**
 * Fields a stored mount may deliberately leave null: no cache ceiling of its own,
 * or rclone's default read ahead.
 */
const NULLABLE_FIELDS: ReadonlySet<string> = new Set(["VfsCacheMaxSizeBytes", "ReadAheadBytes"]);

/**
 * Drops values that would override a default with something the tab cannot use.
 * A null cache mode, from a hand-edited value, reached `mountSignature` and
 * crashed the tab on `toLowerCase`.
 */
function withoutUnusableValues(entry: Partial<BuiltinMount>): Partial<BuiltinMount> {
  return Object.fromEntries(
    Object.entries(entry).filter(([key, value]) => {
      if (value === null) return NULLABLE_FIELDS.has(key);
      return key !== "VfsCacheMode" || typeof value === "string";
    }),
  );
}
