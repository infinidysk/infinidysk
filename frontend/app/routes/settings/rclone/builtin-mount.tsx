...
export function BuiltinMountSettings({ config, setNewConfig }: BuiltinMountSettingsProps) {
  ...
  // Without the saved list there is no telling whether the edits on screen are
  // saved, so Apply waits for it -- on the first load and after a failed poll.
  const mountStatusLoaded = status?.mounts != null;
        ...
        >
          <div className="flex flex-col gap-3">
                      ...
                      <label className="space-y-1">
                        ...
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            const typed = e.target.value.trim();
                            if (typed === readAheadDisplay(mount.ReadAheadBytes)) return;

                            // Empty clears the setting. A value that cannot be
                            // read keeps the stored one, as the cache size limit
                            // does, rather than silently wiping it.
                            const parsed = typed === "" ? null : parseSize(typed);
                            if (typed !== "" && parsed === null) return;
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                onClick={() => void apply()}
                disabled={busy === "applying" || !mountStatusLoaded || hasUnsavedMounts}
              >
