...
describe("parseMounts", () => {
  ...
  // A hand-edited value can carry nulls. One in a field that has a default used
  // to reach mountSignature and crash the tab on toLowerCase; the nulls that mean
  // "no ceiling" or "rclone's default" are kept.
  it("falls back to the default for a null field that has one", () => {
    const [parsed] = parseMounts(
      '[{"Id":"library","MountPoint":"/mnt/remote","VfsCacheMode":null,"Enabled":null,"ReadAheadBytes":null,"VfsCacheMaxSizeBytes":null}]',
    );

    expect(parsed!.VfsCacheMode).toBe("full");
    expect(parsed!.Enabled).toBe(true);
    expect(parsed!.ReadAheadBytes).toBeNull();
    expect(parsed!.VfsCacheMaxSizeBytes).toBeNull();
    expect(() => mountsMatchServer([parsed!], [])).not.toThrow();
  });
});
