...
describe("BuiltinMountSettings", () => {
  ...
  // A typo must not wipe the stored value: the box would keep showing the typo
  // while the mount quietly fell back to rclone's default.
  it("keeps read ahead when the typed value cannot be read", async () => {
    let saved: Record<string, string> = {};
    respondWith({
      status: true,
      enabled: true,
    ...
    const [kept] = JSON.parse(saved["rclone.builtin.mounts"] ?? "[]") as BuiltinMount[];
    expect(kept?.ReadAheadBytes).toBe(12345678);
  });

  it("still clears read ahead when the field is emptied", async () => {
    let saved: Record<string, string> = {};
    respondWith({
      status: true,
      enabled: true,
    ...
    await waitFor(() => {
      const [cleared] = JSON.parse(saved["rclone.builtin.mounts"] ?? "[]") as BuiltinMount[];
      expect(cleared?.ReadAheadBytes ?? null).toBeNull();
    });
  });

  // Until the saved list arrives there is nothing to compare the screen with,
  // and not knowing must not read as "nothing unsaved".
  it("keeps Apply disabled until the saved mount list has loaded", async () => {
  ...
  it("keeps Apply disabled when the mount status cannot be read", async () => {
    fetchMock.mockImplementation(() => Promise.reject(new Error("backend down")));
