...
import {
  adjustTotalCount,
  applyQueueProvidersMessage,
  parseQueueProvidersPayload,
...
describe("applyQueueProvidersMessage", () => {
  it("preserves nicknames from the previous provider list", () => {
    const result = applyQueueProvidersMessage("slot-1|news.newsgroup.ninja=12", [
  ...
  it("matches hosts case-insensitively when preserving nicknames", () => {
    const result = applyQueueProvidersMessage("slot-1|NEWS.NEWSGROUP.NINJA=4", [
  ...
  it("clears providers when the payload is empty", () => {
    const result = applyQueueProvidersMessage("slot-1|", [
      { host: "news.example.com", nickname: "Main", segments: 1 },
    ]);
