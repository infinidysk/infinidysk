...
export function parseQueueProvidersMessage(
  message: string,
): { nzo_id: string; payload: string } | null {
...
export function useQueueEvents(
  ...
  const onAddQueueSlot = useCallback(
    (queueSlot: QueueSlot) => {
      uploadQueueRef.current = uploadQueueRef.current.filter(
        (x) => x.queueSlot.status === "uploading" || x.queueSlot.filename !== queueSlot.filename,
      );
      setUploadingFiles((files) =>
        files.filter((f) => f.queueSlot.filename !== queueSlot.filename),
      );
  ...
  const onSelectQueueSlots = useCallback(
    (ids: Set<string>, isSelected: boolean) => {
      setUploadingFiles((files) =>
        files.map((x) =>
          ids.has(x.queueSlot.nzo_id) ? { ...x, queueSlot: { ...x.queueSlot, isSelected } } : x,
        ),
      );
  ...
  const onRemovingQueueSlots = useCallback(
    (ids: Set<string>, isRemoving: boolean) => {
      setQueueSlots((slots) => slots.map((x) => (ids.has(x.nzo_id) ? { ...x, isRemoving } : x)));
    },
    [setQueueSlots],
  );

  const onRemoveQueueSlots = useCallback(
    (ids: Set<string>) => {
      uploadQueueRef.current = uploadQueueRef.current.filter(
        (x) => x.queueSlot.status === "uploading" || !ids.has(x.queueSlot.nzo_id),
      );
      setUploadingFiles((files) =>
        files.filter((x) => x.queueSlot.status === "uploading" || !ids.has(x.queueSlot.nzo_id)),
      );
  ...
  const onMoveQueueSlotsToTop = useCallback(
    (ids: Set<string>) => {
      ...
      setQueueSlots((slots) => {
        ...
        return [...remaining.slice(0, index), ...moved, ...remaining.slice(index)].slice(
          0,
          pageSize,
        );
      });
  ...
  const onChangeQueueSlotStatus = useCallback(
    (message: string) => {
  ...
  const onChangeQueueSlotPercentage = useCallback(
    (message: string) => {
      const [nzo_id = "", true_percentage = ""] = message.split("|");
      setQueueSlots((slots) =>
        slots.map((x) => (x.nzo_id === nzo_id ? { ...x, true_percentage } : x)),
      );
    },
    [setQueueSlots],
  );

  const onChangeQueueSlotProviders = useCallback(
    (message: string) => {
      const parsed = parseQueueProvidersMessage(message);
      if (!parsed) return;
...
export function useHistoryEvents(
  ...
  const onAddHistorySlot = useCallback(
  ...
  const onSelectHistorySlots = useCallback(
    (ids: Set<string>, isSelected: boolean) => {
  ...
  const onRemovingHistorySlots = useCallback(
    (ids: Set<string>, isRemoving: boolean) => {
  ...
  const onRemoveHistorySlots = useCallback(
    (ids: Set<string>) => {
