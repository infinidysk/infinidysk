...
export function QueueTable({
  ...
  const sectionTitle = (
    <div className="flex items-center gap-2.5">
      ...
      <div className="ml-2.5 hidden min-[450px]:block">{categoryDropdown}</div>
    </div>
  );

  const sectionSubTitle = <div className="block min-[450px]:hidden">{categoryDropdown}</div>;
...
export const QueueRow = memo(
  ({ slot, onIsSelectedChanged, onIsRemovingChanged, onRemoved, onMovedToTop }: QueueRowProps) => {
    const isReadOnly = useIsReadOnly();
    // state
    const [isConfirmingRemoval, setIsConfirmingRemoval] = useState(false);
