export function hasConfiguredArrs(configValue?: string): boolean {
  ...
  try {
    ...
    const radarr: unknown[] = Array.isArray(record["RadarrInstances"])
      ? record["RadarrInstances"]
      : [];
    const sonarr: unknown[] = Array.isArray(record["SonarrInstances"])
      ? record["SonarrInstances"]
      : [];
