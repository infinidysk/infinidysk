...
public class SymlinkRewriterTests
{
    ...
    private static async Task<(Guid Id, string NewTarget)> SeedDavTargetAsync(
        MigrationTestHarness h, string? mountDir = null)
    {
        var id = Guid.NewGuid();
        var mount = mountDir ?? new ConfigManager().GetRcloneMountDir();
        var newTarget = DatabaseStoreSymlinkFile.GetTargetPath(id, mount, '/');
        await using var dav = h.Dav();
        dav.Items.Add(DavItem.New(
            id, DavItem.ContentFolder, $"{id:N}.mkv", 1,
    ...
    private static SymlinkRewriter CreateRewriter(MigrationTestHarness h, ISymlinkOps ops, ConfigManager? config = null) =>
        new(h.Store, config ?? new ConfigManager())
        {
            Ops = ops,
    ...
    public async Task Apply_RetargetsRewrites_LeavesOrphansAndOthersUntouched_AndBacksUpFirst()
    {
        ...
        await SeedPlanAsync(h,
            Row("rewrite", "/lib/a.mkv", "/mnt/altmount/tv/a.mkv", newTarget),
        ...
        var rewriter = CreateRewriter(h, ops);
        ...
        Assert.Equal(newTarget, ops.Links["/lib/a.mkv"]);
        ...
        Assert.Equal(newTarget, entry.ReplacementTarget);
    ...
    public async Task Apply_DriftGuard_FailsWhenTargetChangedSincePlan()
    {
        ...
        var summary = await CreateRewriter(h, ops).ApplyAsync();
    ...
    [Fact]
    public async Task Apply_FailsWhenDavItemDeleted_LeavesDiskUntouched()
    {
        await using var h = await MigrationTestHarness.CreateAsync();
        ...
        await using (var dav = h.Dav())
        {
            var item = await dav.Items.SingleAsync(i => i.Id == id);
            dav.Items.Remove(item);
            await dav.SaveChangesAsync();
        }
    ...
    [Fact]
    public async Task Apply_FailsWhenMountDirChanged_LeavesDiskUntouched()
    {
        await using var h = await MigrationTestHarness.CreateAsync();
        ...
        config.UpdateValues(
        [
            new ConfigItem { ConfigName = ConfigKeys.RcloneMountDir, ConfigValue = "/mnt/other-mount" },
        ]);
        var ops = new FakeSymlinkOps { Links = { ["/lib/a.mkv"] = "/mnt/altmount/tv/a.mkv" } };
        var summary = await CreateRewriter(h, ops, config).ApplyAsync();
