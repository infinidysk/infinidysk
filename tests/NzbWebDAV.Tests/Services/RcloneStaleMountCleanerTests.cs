...
public class RcloneStaleMountCleanerTests
{
    ...
    [Fact]
    public void IsResponsiveWithin_AsksAgain_OnceAProbeThatOverranItsBudgetAnswers()
    {
        // A wedged mount can recover. Its probe then answers with nobody waiting,
        // and reusing that old verdict on the next sweep would skip the read that
        // shows how the mount is doing now.
        const string mountPoint = "/data/recovers";
        using var blocked = new ManualResetEventSlim(false);
        var started = 0;

        Assert.True(RcloneStaleMountCleaner.IsResponsiveWithin(
            mountPoint,
            () =>
            {
                Interlocked.Increment(ref started);
        ...
        Assert.True(
            SpinWait.SpinUntil(
                () => !RcloneStaleMountCleaner.HasOutstandingProbe(mountPoint),
                TimeSpan.FromSeconds(5)),
            "the probe that overran its budget kept its entry after it answered");
