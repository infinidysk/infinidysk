...
public class RcloneImportServiceTests
{
    ...
    [Fact]
    public async Task PreviewAsync_WarnsWhenTheExternalInstanceReportsNoVfsOptions()
    {
        // A successful answer with no options in it leaves the mount on defaults,
        // which must not pass for a faithful copy of the external tuning.
        var external = new FakeExternalRclone
        {
            Live = [new RcloneMountPoint { Fs = "nzbdav:", MountPoint = "/mnt/remote/nzbdav" }],
            Options = null,
        };
        ...
        Assert.Single(preview.Mounts);
        Assert.Contains(
            preview.Warnings,
            w => w.Contains("Could not read the VFS settings", StringComparison.Ordinal));
    }
