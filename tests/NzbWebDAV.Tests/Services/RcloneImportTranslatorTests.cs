...
public class RcloneImportTranslatorTests
{
    ...
    [Theory]
    [InlineData("/mnt/media-a", "/mnt/media/a")]
    [InlineData("/", "/root")]
    [InlineData("/mnt/Media", "/mnt/media")]
    public void DeriveId_KeepsDistinctMountPointsApart(string first, string second)
    {
        // Joining the path segments with dashes made these pairs one id, which an
        // import of both then failed as a duplicate. Validation compares ids
        // without case, so paths that differ only in case must differ beyond it.
        var firstId = RcloneImportTranslator.DeriveId(first);
        var secondId = RcloneImportTranslator.DeriveId(second);

        Assert.NotEqual(firstId, secondId, StringComparer.OrdinalIgnoreCase);
        Assert.Empty(RcloneMountConfig.Validate(
            [
                new RcloneMountConfig { Id = firstId, MountPoint = "/mnt/one", RemotePath = "/a" },
                new RcloneMountConfig { Id = secondId, MountPoint = "/mnt/two", RemotePath = "/b" },
            ],
            "/config"));
    }

    [Fact]
    public void DeriveId_IsTheSameForTheSamePathHoweverItIsWritten()
    {
        Assert.Equal(
            RcloneImportTranslator.DeriveId("/mnt/remote/nzbdav"),
            RcloneImportTranslator.DeriveId("/mnt//remote/nzbdav/"));
    }
