...
public class RcloneDaemonServiceTests : IDisposable
{
    // Starting the daemon publishes its client process-wide, and a test that
    // leaves it behind hands the next one a daemon it never started.
    public void Dispose() => RcloneClient.Builtin = null;
