...
public class RcloneMountsStatusBuilderTests
{
    ...
    [Fact]
    public void Build_ReportsALiveMountServingADifferentRemoteSeparately()
    {
        // The configured mount point is taken, but by another remote path. Calling
        // the configured mount mounted would hide that its remote is not what is
        // being served there.
        var configured = Configured();
        configured.RemotePath = "/content";

        var status = RcloneMountsStatusBuilder.Build(
            Running(),
            [configured],
            [new RcloneMountPoint { Fs = "infinidysk:/old", MountPoint = "/mnt/remote" }]);

        Assert.Equal(2, status.Mounts.Count);
        Assert.False(Assert.Single(status.Mounts, m => m.Configured).Mounted);

        var live = Assert.Single(status.Mounts, m => !m.Configured);
        Assert.Equal("/mnt/remote", live.MountPoint);
        Assert.Equal("/old", live.RemotePath);
        Assert.True(live.Mounted);
    }
