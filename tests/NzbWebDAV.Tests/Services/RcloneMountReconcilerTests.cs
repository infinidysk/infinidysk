...
public class RcloneMountReconcilerTests
{
    ...
    [Fact]
    public async Task ReconcileAsync_SplitsTheInstallWideCeilingAcrossEnabledMounts()
    {
        // rclone applies the ceiling per mount. Handing each mount the whole limit
        // lets two mounts fill twice the space the operator allowed. A disabled
        // mount is never mounted, so it takes no share.
        var client = new FakeRcloneClient { Remotes = ["infinidysk"] };
        var budget = new RcloneCacheBudget
        {
            AvailableFreeSpace = _ => 400L * 1024 * 1024 * 1024,
        ...
        var reconciler = new RcloneMountReconciler(
            client, budget, "/mnt/fast/cache", "/config", 8L * 1024 * 1024 * 1024);
        var movies = Mount("movies", "/mnt/movies");
        movies.RemotePath = "/movies";
        var tv = Mount("tv", "/mnt/tv");
        tv.RemotePath = "/tv";
        var retired = Mount("retired", "/mnt/retired");
        retired.RemotePath = "/retired";
        retired.Enabled = false;

        await reconciler.ReconcileAsync([movies, tv, retired], CancellationToken.None);

        Assert.Equal(2, client.Mounted.Count);
        Assert.All(client.Mounted, mount => Assert.Equal(4L * 1024 * 1024 * 1024, mount.VfsOpt!["CacheMaxSize"]));
    }

    [Fact]
    public async Task ReconcileAsync_SplitsTheAutomaticCeilingAcrossEnabledMounts()
    {
        // The automatic budget is sized for the whole cache volume, headroom for
        // the databases included; it only stays within that if the mounts share it.
        var client = new FakeRcloneClient { Remotes = ["infinidysk"] };
        var budget = new RcloneCacheBudget
        {
            AvailableFreeSpace = _ => 400L * 1024 * 1024 * 1024,
        ...
        await reconciler.ReconcileAsync([movies, tv], CancellationToken.None);

        Assert.Equal(2, client.Mounted.Count);
        Assert.All(client.Mounted, mount => Assert.Equal(RcloneCacheBudget.DefaultCapBytes / 2, mount.VfsOpt!["CacheMaxSize"]));
    }

    [Fact]
    public async Task ReconcileAsync_LeavesMountsAlone_WhileTheyRunWithTheirShareOfTheCeiling()
    {
        // Each mount runs with its share, so comparing the live value with the
        // whole limit would remount every mount on every pass.
        var shareOptions = new VfsOptions
        {
            CacheMode = "full",
            Links = true,
            ...
            CacheMaxSize = 4L * 1024 * 1024 * 1024,
        };
        var client = new FakeRcloneClient
        {
            Remotes = ["infinidysk"],
            Live =
            [
                new RcloneMountPoint { Fs = "infinidysk:/movies", MountPoint = "/mnt/movies" },
                new RcloneMountPoint { Fs = "infinidysk:/tv", MountPoint = "/mnt/tv" },
            ],
            LiveOptions = shareOptions,
        };
        var reconciler = new RcloneMountReconciler(client, cacheSizeLimitBytes: 8L * 1024 * 1024 * 1024);
        var movies = Mount("movies", "/mnt/movies");
        movies.RemotePath = "/movies";
        var tv = Mount("tv", "/mnt/tv");
        tv.RemotePath = "/tv";

        var result = await reconciler.ReconcileAsync([movies, tv], CancellationToken.None);
