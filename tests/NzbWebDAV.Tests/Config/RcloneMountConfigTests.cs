...
public class RcloneMountConfigTests
{
    ...
    public void Validate_RejectsDuplicateIds()
    {
        var mounts = new List<RcloneMountConfig>
        {
            new() { Id = "library", MountPoint = "/mnt/one", RemotePath = "/movies" },
            new() { Id = "library", MountPoint = "/mnt/two", RemotePath = "/tv" },
        };
    ...
    public void Validate_RejectsTwoMountsOnTheSamePath()
    {
        var mounts = new List<RcloneMountConfig>
        {
            new() { Id = "one", MountPoint = "/mnt/remote", RemotePath = "/movies" },
            new() { Id = "two", MountPoint = "/mnt/remote/", RemotePath = "/tv" },
        };
    ...
    public void Validate_AllowsSiblingMounts()
    {
        var errors = RcloneMountConfig.Validate(
            [
                new RcloneMountConfig { Id = "a", MountPoint = "/mnt/remote", RemotePath = "/movies" },
                new RcloneMountConfig { Id = "b", MountPoint = "/mnt/remote-other", RemotePath = "/tv" },
            ],
            "/config");
    ...
    public void Validate_RejectsTwoEnabledMountsOfTheSameRemotePath(string first, string second)
    {
        // rclone names a VFS after its remote and refuses a vfs/forget that names
        // one two mounts share, so their directory listings could never be dropped.
        var errors = RcloneMountConfig.Validate(
            [
                new RcloneMountConfig { Id = "a", MountPoint = "/mnt/one", RemotePath = first },
                new RcloneMountConfig { Id = "b", MountPoint = "/mnt/two", RemotePath = second },
            ],
            "/config");

        var error = Assert.Single(errors);
        Assert.Contains("already served by mount 'a'", error, StringComparison.Ordinal);
    }

    [Fact]
    public void Validate_AllowsASharedRemotePath_WhenOneMountIsDisabled()
    {
        // A disabled mount is never mounted, so it creates no second VFS.
        var errors = RcloneMountConfig.Validate(
            [
                new RcloneMountConfig { Id = "a", MountPoint = "/mnt/one" },
                new RcloneMountConfig { Id = "b", MountPoint = "/mnt/two", Enabled = false },
            ],
            "/config");
