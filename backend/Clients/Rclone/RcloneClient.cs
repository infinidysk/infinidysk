...
public sealed class RcloneClient : IRcloneClient, IDisposable
{
    ...
    /// <summary>
    /// Forget (clear) VFS directory cache entries on the only VFS. Kept for callers
    /// that pass the cancellation token positionally.
    /// </summary>
    public Task<VfsForgetResponse> ForgetVfsPaths(IEnumerable<string> paths, CancellationToken cancellationToken) =>
