    ...
    protected override async Task<IActionResult> HandleRequest()
    {
        ...
        var outcome = await daemonService.WithMountGateAsync<(string? Error, RcloneReconcileResult? Result)>(
            async token =>
            {
                ...
                if (!unmount.Success)
                {
                    // The detail goes to the server log. The response carries a
                    // fixed message rather than the mount path and rclone's text.
                    Log.Warning(
                        "Could not unmount {MountPoint} for a remount: {Error}",
                        mount.MountPoint,
                        unmount.Error ?? "unknown error");
                    return ("Could not unmount the mount. Check the server log for details.", null);
                }
