...
public sealed class RcloneImportService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var mountPoint in mountPoints)
        {
            ...
            // A successful answer that carries no options is no better than a
            // failed one: the defaults stand in for it, and the user is told so.
            if (!stats.Success || stats.Options is null)
            {
                warnings.Add(
