...
public static class RcloneMountsStatusBuilder
{
        ...
        bool remoteConfigured = false)
    {
        ...
        // listing might say.
        var liveMounts = daemon.Running
            ? live.Where(m => m.MountPoint is not null).ToList()
            : [];

        // A configured mount is mounted only when its own remote is what is
        // mounted there. Matching by path alone reported a mount point serving a
        // different remote as healthy, and hid the record of what really is.
        static bool Serves(RcloneMountPoint liveMount, RcloneMountConfig mount) =>
            string.Equals(liveMount.MountPoint, mount.MountPoint, StringComparison.Ordinal)
            && string.Equals(
                RcloneMountReconciler.NormalizeFs(liveMount.Fs),
                RcloneMountReconciler.NormalizeFs(RcloneMountReconciler.BuildFs(mount)),
        ...
        var rows = configured
            .Select(mount => new RcloneMountStatusRow(
                ...
                Configured: true,
                mount.Enabled,
                liveMounts.Any(liveMount => Serves(liveMount, mount)),
                mount.AllowOther,
                mount.Links,
        ...
        // Everything live that no configured mount accounts for, including a
        // configured mount point that is serving some other remote.
        rows.AddRange(liveMounts
            .Where(liveMount => !configured.Any(mount => Serves(liveMount, mount)))
            .Select(liveMount => new RcloneMountStatusRow(
                liveMount.MountPoint!,
                Name: null,
                liveMount.MountPoint!,
                RemotePath: RcloneImportTranslator.ExtractRemotePath(liveMount.Fs),
