...
public sealed class RcloneStaleMountCleaner
{
    ...
    internal static bool IsResponsiveWithin(string mountPoint, Func<bool> probe, TimeSpan budget)
    {
        ...
        var attempt = OutstandingProbes.GetOrAdd(mountPoint, key => StartProbe(key, probe));
    ...
    /// <summary>
    /// Starts a probe whose entry removes itself once it answers. A probe that
    /// overran its budget answers with nobody waiting, and an entry left behind
    /// would hand that old verdict to the next sweep as if it were new.
    /// </summary>
    private static Task<bool> StartProbe(string mountPoint, Func<bool> probe)
    {
        var attempt = Task.Run(probe);
        _ = attempt.ContinueWith(
            finished => OutstandingProbes.TryRemove(new KeyValuePair<string, Task<bool>>(mountPoint, finished)),
    ...
    /// <summary>Whether a probe is still outstanding for a mount point.</summary>
    internal static bool HasOutstandingProbe(string mountPoint) => OutstandingProbes.ContainsKey(mountPoint);
