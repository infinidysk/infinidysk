    ...
    long? cacheSizeLimitBytes = null)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // rclone applies a cache ceiling per mount, while the limit is one budget
        // for the whole cache, so every enabled mount gets an equal share of it.
        var cacheShares = Math.Max(1, wanted.Count);
        ...
        foreach (var mount in wanted.Values)
        {
            ...
            if (!await LiveTuningMatchesAsync(mount, cacheShares, cancellationToken).ConfigureAwait(false))
        ...
        foreach (var (mountPoint, liveFs) in livePoints)
        {
            ...
            // Checked before every change to rclone, not only on entry: shutdown
            // cancels a pass in flight, and a change issued after the shutdown
            // release would put back a mount that is then left behind.
        ...
        foreach (var config in toMount)
        {
            ...
            var response = await client
                .MountFs(BuildFs(config), config.MountPoint, BuildMountOptions(config), BuildVfsOptions(config, cacheShares), cancellationToken)
    ...
    /// </remarks>
    private async Task<bool> LiveTuningMatchesAsync(
        RcloneMountConfig config,
        int cacheShares,
        CancellationToken cancellationToken)
    {
        ...
        if (ExplicitCacheCeiling(config, cacheShares) is { } ceiling && live.CacheMaxSize != ceiling) return false;
    ...
    private Dictionary<string, object?> BuildVfsOptions(RcloneMountConfig config, int cacheShares)
    {
        ...
        if (ResolveCacheMaxSize(config, cacheShares) is { } maxSize) options["CacheMaxSize"] = maxSize;
    ...
    /// <summary>
    /// The ceiling somebody set for this mount: its share of the install-wide
    /// limit, or else the mount's own.
    /// </summary>
    /// <remarks>
    /// The install-wide limit wins over a per-mount one. The per-mount value only
    /// ever arrives by importing an external rclone, and it is not shown anywhere
    /// in the UI; leaving it to override the box the operator can actually see
    /// would make that box look broken. It is kept whole, as imported.
    /// </remarks>
    private long? ExplicitCacheCeiling(RcloneMountConfig config, int cacheShares) =>
        cacheSizeLimitBytes is { } limit ? ShareOf(limit, cacheShares) : config.VfsCacheMaxSizeBytes;

    /// <summary>
    /// One mount's slice of a cache budget every enabled mount draws on. rclone
    /// enforces CacheMaxSize per mount, so handing each the whole figure would let
    /// N mounts fill N times the space. Never zero, which rclone reads as unlimited.
    /// </summary>
    internal static long ShareOf(long total, int shares) =>
        Math.Max(RcloneCacheBudget.MinimumBytes, total / Math.Max(1, shares));
    ...
    /// </summary>
    private long? ResolveCacheMaxSize(RcloneMountConfig config, int cacheShares)
    {
        var configured = ExplicitCacheCeiling(config, cacheShares);

        if (cacheBudget is null || cacheDir is null || databaseDir is null)
            return configured;

        // The automatic budget is sized for the whole volume, database headroom
        // included, so the mounts split it just as they split an explicit limit.
        var resolved = configured is > 0
            ? configured.Value
            : ShareOf(cacheBudget.ResolveFor(cacheDir, databaseDir, configuredBytes: null), cacheShares);
