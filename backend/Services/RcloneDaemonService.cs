...
public sealed class RcloneDaemonService(
    ...
    IRcloneProcessLauncher launcher) : BackgroundService, IHostedLifecycleService
{
    ...
    /// <summary>Set once shutdown has begun; from then on nothing is mounted.</summary>
    private volatile bool _shuttingDown;

    /// <summary>Set once the release has been attempted, so shutdown does not repeat it.</summary>
    private volatile bool _mountsReleased;

    /// <summary>
    /// Cancelled when shutdown begins, so a mount pass already under way stops
    /// changing rclone instead of racing the release.
    /// </summary>
    private readonly CancellationTokenSource _shutdown = new();

    /// <summary>
    /// How long shutdown waits for a cancelled mount pass to give the gate back
    /// before it leaves the mounts to the process stop. Seam for tests.
    /// </summary>
    internal TimeSpan ShutdownGateTimeout { get; init; } = UnmountTimeout;
    ...
    /// <summary>
    /// Releases the mounts before any hosted service stops.
    ///
    ...
    /// </summary>
    /// <remarks>
    /// The host calls this ahead of every <see cref="IHostedService.StopAsync"/>,
    /// the web server's included. Registration order alone cannot put this
    /// service behind the web host, which ASP.NET adds when the app is built, and
    /// the web server's stop waits for open requests -- rclone's own reads among
    /// them -- on the same shutdown budget the unmount needs.
    /// </remarks>
    public Task StoppingAsync(CancellationToken cancellationToken) =>
        ReleaseMountsForShutdownAsync(ShutdownGateTimeout);
    ...
    /// <summary>
    /// Stops the supervisor and then the process. The mounts are normally
    /// released already, by <see cref="StoppingAsync"/>; a caller that stops this
    /// service directly still has them released first, and a release shutdown
    /// had to skip is tried once more without waiting again.
    /// </summary>
    public override async Task StopAsync(CancellationToken cancellationToken)
    {
        if (!_mountsReleased)
        {
            await ReleaseMountsForShutdownAsync(_shuttingDown ? TimeSpan.Zero : ShutdownGateTimeout)
    ...
    /// <summary>
    /// Ends mount changes for good, then releases the mounts once no pass holds
    /// the gate.
    /// </summary>
    private async Task ReleaseMountsForShutdownAsync(TimeSpan gateTimeout)
    {
        // Recorded before anything else, so no pass that starts from here on puts
        // back what is about to be released; the pass under way is cancelled, and
        // checks that before each change it would still make to rclone.
        _shuttingDown = true;
        await _shutdown.CancelAsync().ConfigureAwait(false);

        // Unmounting while a pass still holds the gate could race a mount it has
        // already sent, so the mounts stay until it gives the gate back. A pass
        // that does not do so in time is not waited for: a wedged pass must not
        // hold the shutdown open, and the process stop still lets rclone clean up.
        if (!await _mountGate.WaitAsync(gateTimeout, CancellationToken.None).ConfigureAwait(false))
        {
            Log.Warning(
                "A mount pass was still running at shutdown, so the built-in rclone mounts were not " +
                "released. A mount may need 'fusermount3 -uz' before the next start.");
            return;
        }

        try
        {
            await ReleaseMountsAsync().ConfigureAwait(false);
        }
        finally
        {
            _mountGate.Release();
    ...
    private async Task ReleaseMountsAsync()
    {
        _shuttingDown = true;
        ...
        if (client is not null)
        {
            ...
            {
                // Shutdown must continue regardless; the process stop that follows
                // still gives rclone its own chance to clean up.
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Once shutdown has released the mounts, putting one back would leave
            // it behind when the process goes.
            if (_shuttingDown)
                throw new OperationCanceledException("The built-in rclone daemon is shutting down.");

            // Linked to shutdown as well, so an operation already running here is
            // cancelled when shutdown begins rather than going on to mount.
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // Shutdown cancelled this pass on purpose. The loop keeps running
                // until StopAsync, and every pass from here returns at once;
                // leaving now would stop the daemon under the release.
            }
            catch (Exception e)
            {
    ...
    internal async Task ReconcileAsync(CancellationToken cancellationToken)
    {
        ...
        // The supervisor polls until StopAsync, which comes after the mounts were
        // released in StoppingAsync. Neither the daemon nor its mounts are touched
        // in between.
