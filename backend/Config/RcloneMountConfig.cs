...
public class RcloneMountConfig
{
    ...
    public static IReadOnlyList<string> Validate(IEnumerable<RcloneMountConfig> mounts, string configPath)
    {
        ...
        var seenRemotes = new Dictionary<string, string>(StringComparer.Ordinal);
        ...
        foreach (var mount in configured.Where(mount => mount is not null && mount.Enabled))
        {
            ...
            // rclone names a VFS after its remote, and refuses a vfs/forget that
            // names one two mounts share. Two enabled mounts of the same remote
            // path would keep directory listings that nothing could drop.
            var remote = RcloneMountReconciler.NormalizeFs(RcloneMountReconciler.BuildFs(mount));
            if (!seenRemotes.TryAdd(remote, label))
            {
                errors.Add(
                    $"Mount '{label}': remote path '{mount.RemotePath}' is already served by mount " +
                    $"'{seenRemotes[remote]}'. Give each mount a different remote path.");
            }
